...
export function WorkflowDragLayer({ getCardRefs }: WorkflowDragLayerProps) {
  const { isDragging, draggingWorkflowIds, dragPosition } = useDragContext()
  const layerRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (!isDragging || !layerRef.current || draggingWorkflowIds.length === 0) return

    const layer = layerRef.current
    layer.innerHTML = ''

    const cardRefs = getCardRefs()
    const stackCount = Math.min(draggingWorkflowIds.length, 5)
    const offset = 15

    const firstCard = cardRefs.get(draggingWorkflowIds[0])
    if (!firstCard) return
    ...
    for (let i = stackCount - 1; i >= 0; i--) {
      const cardId = draggingWorkflowIds[i]
    ...
    if (draggingWorkflowIds.length > 1) {
      ...
      badge.textContent = String(draggingWorkflowIds.length)
  ...
  }, [isDragging, draggingWorkflowIds, getCardRefs])
